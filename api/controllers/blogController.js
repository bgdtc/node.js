...
exports.getID = async (req, res) => {
  ...
  let comsIDd = await query(`SELECT user.nickname,comments.update_date, comments.content, comments.id FROM user LEFT JOIN comments ON user.id = comments.author_id WHERE article_id = ${req.params.id}`)
  const sql = `SELECT user.is_verified FROM user WHERE id = '${req.session.user.id}';`;
